...
#Custom exception for scripting-related errors.
...
class ScriptingElement:
    ...
    def __post_init__(self):
        ...
        if not any([self.scenarios, self.platforms, self.pre_script, self.post_flash_script, self.post_script]):
            raise ScriptingException("At least one of the properties must be specified")
...
class ScriptingData:
    ...
    def load_from_yaml(cls, filename: Path | str) -> ScriptingData:
        try:
            ...
            raise ScriptingException('Failed to load scripting data') from e
...
def _matches_element(element: str, patterns: list[re.Pattern]) -> bool:
    return any(pattern.fullmatch(element) for pattern in patterns)
