...
class ScriptingData:
    ...
    def find_matching_scripting(self, scenario: str, platform: str) -> ScriptingElement | None:
        ...
        # Iterate through all elements to find those that match the scenario and platform
        ...
        # Check for elements with the 'override_script' parameter
        override_elements = []
        for elem in matching_elements:
            if (elem.pre_script and elem.pre_script.override_script) or \
            ...
            (elem.post_script and elem.post_script.override_script):
                override_elements.append(elem)
                
        if len(override_elements) > 1:
            raise ValueError("Multiple elements with 'override_script' parameter found.")
        elif len(override_elements) == 1:
            return override_elements[0]
