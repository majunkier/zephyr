...
class ScriptingData:
    ...
    def find_matching_scripting(self, scenario: str, platform: str) -> ScriptingElement | None:
        ...
        for element in self.elements:
            ...
            for seen_element in seen_elements:
                if (_matches_element(scenario, seen_element.re_scenarios) and
                    _matches_element(platform, seen_element.re_platforms)):
                    if element.override_script and not seen_element.override_script:
                        continue
                    elif not element.override_script and seen_element.override_script:
                        return seen_element
                    else:
                        logger.error(f"Duplicate scenario '{scenario}' for platform '{platform}' detected.")
                        sys.exit(1)
            return element
        return None
...
def _matches_element(value, pattern):
    # Dummy implementation for matching
